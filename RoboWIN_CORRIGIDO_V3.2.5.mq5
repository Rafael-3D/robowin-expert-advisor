//+------------------------------------------------------------------+
//|                     RoboWIN - VERSÃO CORRIGIDA V3.2.5            |
//|                     ✅ V3.2.4 + Prioriza Resultado > Motivo       |
//|                                                                  |
//|                     HISTÓRICO:                                   |
...
//|                     V3.2.3: +Filtro Proximidade (50pts)          |
//|                     V3.2.4: +Otimização Logs (terminal limpo)    |
//|                     V3.2.5: #6 Prioriza Resultado > Motivo       |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026 - V3.2.5 Prioriza Resultado > Motivo"
...
input int    distanciaProximidade = 50; // ⭐ V3.2.3: Distância mínima para pendurar ordem (pts)
...
//    Níveis: 0=nenhum, 1=BE(+350), 2=+150(+550), 3=+300(+650), 4=+500(+760)
int nivelTrailing = 0;
...
{
    Print("╔═══════════════════════════════════════════════════════════╗");
    Print("║     ROBÔ WIN - VERSÃO CORRIGIDA V3.2.5                    ║");
    Print("║     ✅ V3.2.4 + Prioriza Resultado > Motivo               ║");
    ...
    Print("║     🛡️ #5 Filtro Proximidade (", distanciaProximidade, " pts)                  ║");
    Print("║     🛡️ #6 Prioriza Resultado > Motivo (NOVO)              ║");
...
void OnTick()
{
    ...
    //--- Verificar se há posição aberta DO NOSSO ROBÔ (Filtro por Magic Number)
    bool temNossaPosicao = false;
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ulong ticketPos = PositionGetTicket(i);
        if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 12345) {
            temNossaPosicao = true;
            break; // A posição foi selecionada com sucesso
        }
    }
...
int CancelarTodasOrdensPendentes()
{
    ...
    for (int i = OrdersTotal() - 1; i >= 0; i--) {
        ...
        // Verifica o Símbolo E o Magic Number do RoboWIN
        if (OrderSelect(ticket) && OrderGetString(ORDER_SYMBOL) == _Symbol && OrderGetInteger(ORDER_MAGIC) == 12345) {
...
void AjustarStopsAposExecucao()
{
    ...
    if (trade.PositionModify(_Symbol, novoSL, novoTP)) {
        ...
        uint codigoErro = trade.ResultRetcode();
        ...
        if (tentativasModifyPosicao >= MAX_TENTATIVAS_MODIFY) {
            ...
            // Só encerra o dia se a posição for fechada com sucesso
            if (trade.PositionClose(_Symbol)) {
                diaEncerrado = true;
                tentativasModifyPosicao = 0;
                Print("🔴 Dia encerrado pelo KILL SWITCH - posição sem proteção fechada");
            } else {
                Print("🔴 ERRO CRÍTICO: Corretora rejeitou fechamento a mercado. Retentando no próximo tick!");
                tentativasModifyPosicao--; // Força a continuar tentando fechar no próximo tick
            }
            return;
        }
...
bool ValidarParametros()
{
    ...
    Print("   Trailing Stop: Progressivo (350→BE, 550→+150, 650→+300, 760→+500)");
    Print("   Filtro Proximidade: ", distanciaProximidade, " pts");
...
//+------------------------------------------------------------------+
//| ⭐ V3.2.3: Monitorar níveis com filtro de PROXIMIDADE             |
//| Só pendura ordem quando preço está a <= distanciaProximidade pts  |
//+------------------------------------------------------------------+
void MonitorarNiveisEntrada()
{
    ...
    //--- ⭐ V3.2.3: Controle de log de proximidade (máximo 1x por minuto)
    static datetime ultimoLogProximidade = 0;
    bool podeLogarProximidade = (TimeCurrent() - ultimoLogProximidade > 60);
    ...
    if (logStatusAtivo) {
        ...
        if (TimeCurrent() - ultimoLogStatus > 60) {
            Print("📊 Status: Compra=", (compraExecutada ? "✓" : "aguardando"),
                  ...
                  " | Filtro: ", distanciaProximidade, "pts");
    ...
    if (!compraExecutada && !ordemCompraPendente) {
        ...
        if (precoAtual > pontoCompra1) {
            //--- ⭐ V3.2.3: Verificar proximidade antes de pendurar BUY_LIMIT
            double distancia = precoAtual - pontoCompra1;
            if (distancia <= distanciaProximidade) {
                nivelCompra = pontoCompra1;
                Print("📍 [V3.2.3] COMPRA - Preço PRÓXIMO do nível C1!");
                Print("   Preço: ", precoAtual, " | C1: ", pontoCompra1, 
                      " | Distância: ", (int)distancia, " pts <= ", distanciaProximidade, " pts ✓");
            } else {
                // ⭐ V3.2.4: Log de proximidade apenas se debug ativo
                if (logStatusAtivo && podeLogarProximidade) {
                    Print("⏳ [V3.2.3] COMPRA - Aguardando aproximação de C1...",
                          " Preço: ", precoAtual, " | C1: ", pontoCompra1,
                          " | Distância: ", (int)distancia, " pts > ", distanciaProximidade, " pts");
                    ultimoLogProximidade = TimeCurrent();
                }
            }
        } else if (precoAtual > pontoCompra2) {
            //--- ⭐ V3.2.3: Verificar proximidade antes de pendurar BUY_LIMIT (C2)
            double distancia = precoAtual - pontoCompra2;
            if (distancia <= distanciaProximidade) {
                nivelCompra = pontoCompra2;
                Print("📍 [V3.2.3] COMPRA - Preço PRÓXIMO do nível C2!");
                Print("   Preço: ", precoAtual, " | C2: ", pontoCompra2, 
                      " | Distância: ", (int)distancia, " pts <= ", distanciaProximidade, " pts ✓");
            } else {
                // ⭐ V3.2.4: Log de proximidade apenas se debug ativo
                if (logStatusAtivo && podeLogarProximidade) {
                    Print("⏳ [V3.2.3] COMPRA - Aguardando aproximação de C2...",
                          " Preço: ", precoAtual, " | C2: ", pontoCompra2,
                          " | Distância: ", (int)distancia, " pts > ", distanciaProximidade, " pts");
                    ultimoLogProximidade = TimeCurrent();
                }
            }
    ...
    if (!vendaExecutada && !ordemVendaPendente) {
        ...
        if (precoAtual < pontoVenda1) {
            //--- ⭐ V3.2.3: Verificar proximidade antes de pendurar SELL_LIMIT
            double distancia = pontoVenda1 - precoAtual;
            if (distancia <= distanciaProximidade) {
                nivelVenda = pontoVenda1;
                Print("📍 [V3.2.3] VENDA - Preço PRÓXIMO do nível V1!");
                Print("   Preço: ", precoAtual, " | V1: ", pontoVenda1, 
                      " | Distância: ", (int)distancia, " pts <= ", distanciaProximidade, " pts ✓");
            } else {
                // ⭐ V3.2.4: Log de proximidade apenas se debug ativo
                if (logStatusAtivo && podeLogarProximidade) {
                    Print("⏳ [V3.2.3] VENDA - Aguardando aproximação de V1...",
                          " Preço: ", precoAtual, " | V1: ", pontoVenda1,
                          " | Distância: ", (int)distancia, " pts > ", distanciaProximidade, " pts");
                    ultimoLogProximidade = TimeCurrent();
                }
            }
        } else if (precoAtual < pontoVenda2) {
            //--- ⭐ V3.2.3: Verificar proximidade antes de pendurar SELL_LIMIT (V2)
            double distancia = pontoVenda2 - precoAtual;
            if (distancia <= distanciaProximidade) {
                nivelVenda = pontoVenda2;
                Print("📍 [V3.2.3] VENDA - Preço PRÓXIMO do nível V2!");
                Print("   Preço: ", precoAtual, " | V2: ", pontoVenda2, 
                      " | Distância: ", (int)distancia, " pts <= ", distanciaProximidade, " pts ✓");
            } else {
                // ⭐ V3.2.4: Log de proximidade apenas se debug ativo
                if (logStatusAtivo && podeLogarProximidade) {
                    Print("⏳ [V3.2.3] VENDA - Aguardando aproximação de V2...",
                          " Preço: ", precoAtual, " | V2: ", pontoVenda2,
                          " | Distância: ", (int)distancia, " pts > ", distanciaProximidade, " pts");
                    ultimoLogProximidade = TimeCurrent();
                }
            }
...
bool ExecutarOrdemMercado(ENUM_ORDER_TYPE tipo)
{
    ...
    bool resultado;
    if (tipo == ORDER_TYPE_BUY) {
        resultado = trade.Buy(contratos, _Symbol, 0, 0, 0, "WIN-V3.25-Mercado");
    } else {
        resultado = trade.Sell(contratos, _Symbol, 0, 0, 0, "WIN-V3.25-Mercado");
    }
...
//|   Nível 1: Lucro >= 350 pts → SL = Entrada (Break Even)        |
//|   Nível 2: Lucro >= 550 pts → SL = Entrada + 150 pts           |
//|   Nível 3: Lucro >= 650 pts → SL = Entrada + 300 pts           |
...
void GerenciarTrailingStop()
{
    ...
    if (lucro >= 760 && nivelTrailing < 4) {
    ...
    else if (lucro >= 650 && nivelTrailing < 3) {
        ...
        if (posicaoAtual.tipo == POSITION_TYPE_BUY)
            ...
            novoSL = NormalizarPreco(posicaoAtual.precoEntrada - 300);
        novoNivel = 3;
        descricaoNivel = "NÍVEL 3: SL = Entrada + 300 pts (lucro >= 650)";
    }
    else if (lucro >= 550 && nivelTrailing < 2) {
        ...
        if (posicaoAtual.tipo == POSITION_TYPE_BUY)
            ...
            novoSL = NormalizarPreco(posicaoAtual.precoEntrada - 150);
        novoNivel = 2;
        descricaoNivel = "NÍVEL 2: SL = Entrada + 150 pts (lucro >= 550)";
    }
    else if (lucro >= 350 && nivelTrailing < 1) {
        // Nível 1: Break Even
        novoSL = NormalizarPreco(posicaoAtual.precoEntrada);
        novoNivel = 1;
        descricaoNivel = "NÍVEL 1: Break Even (lucro >= 350)";
    }
...
void VerificarResultadoPosicao()
{
    ...
    //--- ⭐ V3.2.5 PRIORIDADE #1: Verificar resultado ANTES do motivo
    //--- Se profit > 0, significa que houve ganho (TP ou Trailing com lucro)
    //--- Neste caso, encerra o dia imediatamente SEM verificar motivo
    if (profit > 0) {
        ...
        // ⭐ STATS: Registrar fechamento com motivo genérico "LUCRO"
        Stats_OnClose(profit, "LUCRO");
        
        // 🔴 PRIORIDADE: Encerrar o dia - NÃO permite nova entrada
        diaEncerrado = true;
        Print("\n🔴 ENCERRANDO DIA - Regra: GANHO = FIM DO DIA");
        Print("   (V3.2.5: Qualquer trade lucrativa encerra operações)");
        CancelarTodasOrdensPendentes();
        EncerrarDia("Trade lucrativa detectada - Ganho = Fim do dia");
        return;  // SAIR DA FUNÇÃO - não fazer mais nenhuma verificação
    }
    
    // Detecção de TP vs SL (apenas para trades com prejuízo)
    bool foiTP = false;
    bool foiSL = false;
    bool foiBE = false;
